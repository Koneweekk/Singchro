...
def sliceRecord():
    OUTPATH = 'user_train/dataset_raw/singer/'
    import librosa
    audio, sr = librosa.load("data_train/record.wav", sr=None, mono=False)
    slicer = Slicer(
        sr=sr,
        threshold=-40,
        min_length=15000,
    ...
    for i, chunk in enumerate(chunks):
        if len(chunk.shape) > 1:
            chunk = chunk.T
        soundfile.write(f'{OUTPATH}{i:06d}.wav', chunk, sr)

    file_list = os.listdir(OUTPATH)
    for file in file_list:
        target = OUTPATH + file
        audio = wave.open(target)
