...
def train(rank, num_gpus, chkpt_path, hp, hp_str):

    if num_gpus > 1:
        init_process_group(backend=hp.dist_config.dist_backend, init_method=hp.dist_config.dist_url,
                            world_size=hp.dist_config.world_size * num_gpus, rank=rank) 
    ...
    if rank == 0:
        pth_dir = os.path.join(hp.log.pth_dir, 'sovits5.0')
        log_dir = os.path.join(hp.log.log_dir, 'sovits5.0')
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(log_dir, '%s-%d.log' % ('sovits5.0', time.time()))),
    ...
    trainloader = create_dataloader_train(hp, num_gpus, rank)
    ...
    for epoch in range(init_epoch, hp.train.epochs):
        ...
        if rank == 0 and epoch % hp.log.save_interval == 0:
            ...
            torch.save({
                ...
                'model_d': (model_d.module if num_gpus > 1 else model_g).state_dict(),
        ...
        if rank == 0:
            def clean_checkpoints(path_to_models=f'{pth_dir}', n_ckpts_to_keep=hp.log.keep_ckpts, sort_by_time=True):
                ...
                name_key = (lambda _f: int(re.compile(f'sovits5.0_(\d+)\.pt').match(_f).group(1)))
